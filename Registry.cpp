/*
From http://www.codeproject.com/system/registry.asp
Robert Pittenger
*/
...
//	char * sz;
	...
	//const char * temp=(const char *)strValue.GetBuffer();
...
/*#ifdef _UNICODE
	...
	if (::RegOpenKeyEx(m_hRootKey, LPCTSTR(m_strCurrentPath), 0, KEY_WRITE, &hKey) != ERROR_SUCCESS)
		return FALSE;
	
//#ifdef _UNICODE
//	if (::RegSetValueEx(hKey, LPCTSTR(strName), 0,
//		REG_SZ, (LPBYTE)strValue.GetBuffer(strValue.GetLength()), strValue.GetLength()+ 1/*strlen(sz)*/ )
//		 != ERROR_SUCCESS) bSuccess = FALSE;
//#else
	if (::RegSetValueEx(hKey, LPCTSTR(strName), 0, REG_SZ, (const BYTE *)strValue.GetString(), (DWORD) (lstrlen(strValue.GetBuffer(strValue.GetLength()))+1)*sizeof(TCHAR)+ 1) != ERROR_SUCCESS)
		bSuccess = FALSE;
//#endif
...
BOOL CRegistry::WriteFloat(CString strName, double fValue)
{
	...
	if (::RegSetValueEx(hKey, LPCTSTR(strName), 0, REG_BINARY, (LPBYTE)&fValue, sizeof(fValue)) != ERROR_SUCCESS)
		bSuccess = FALSE;
